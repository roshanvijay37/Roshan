import { Link2, LogOut, Shield } from "lucide-react";
import { useCallback, useEffect, useState } from "react";
import { authApi, isKiteConnected } from "../services/api";

export function KiteConnect() {
  const [connected, setConnected] = useState(false);
  const [user, setUser] = useState<{ userName: string; broker: string } | null>(null);
  const [loading, setLoading] = useState(false);

  const checkSession = useCallback(async () => {
    const sessionId = localStorage.getItem("kiteSessionId");
    if (!sessionId) {
      setConnected(false);
      setUser(null);
      return;
    }

    try {
      const result = await authApi.checkSession(sessionId);
      if (result.valid) {
        setConnected(true);
        setUser(result.user);
      } else {
        localStorage.removeItem("kiteSessionId");
        setConnected(false);
        setUser(null);
      }
    } catch {
      localStorage.removeItem("kiteSessionId");
      setConnected(false);
      setUser(null);
    }
  }, []);

  useEffect(() => {
    checkSession();

    // Listen for logout events
    const handleLogout = () => {
      setConnected(false);
      setUser(null);
    };
    window.addEventListener("kite:logout", handleLogout);
    return () => window.removeEventListener("kite:logout", handleLogout);
  }, [checkSession]);

  // Handle OAuth callback (request_token in URL)
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const requestToken = params.get("request_token");
    const status = params.get("status");

    if (status === "success" && requestToken) {
      setLoading(true);
      authApi
        .exchangeToken(requestToken)
        .then((result) => {
          localStorage.setItem("kiteSessionId", result.sessionId);
          setConnected(true);
          setUser(result.user);
          // Clean URL
          window.history.replaceState({}, "", window.location.pathname);
        })
        .catch((err) => {
          alert("Zerodha login failed: " + err.message);
        })
        .finally(() => setLoading(false));
    }
  }, []);

  const handleLogin = async () => {
    setLoading(true);
    try {
      const { loginUrl } = await authApi.getLoginUrl();
      window.location.href = loginUrl;
    } catch (err: any) {
      alert("Failed to get login URL: " + err.message);
      setLoading(false);
    }
  };

  const handleLogout = () => {
    authApi.logout();
    setConnected(false);
    setUser(null);
  };

  if (connected && user) {
    return (
      <div className="flex items-center gap-3 rounded-xl border border-lime-400/20 bg-lime-400/10 px-4 py-2.5">
        <Shield size={16} className="text-lime-400" />
        <div className="flex flex-col">
          <span className="text-xs font-medium text-lime-300">
            {user.userName}
          </span>
          <span className="text-[10px] text-lime-400/60">{user.broker}</span>
        </div>
        <button
          onClick={handleLogout}
          className="ml-2 rounded-lg p-1 text-zinc-500 transition hover:bg-zinc-800 hover:text-rose-300"
          title="Disconnect Zerodha"
        >
          <LogOut size={14} />
        </button>
      </div>
    );
  }

  return (
    <button
      onClick={handleLogin}
      disabled={loading}
      className="flex items-center gap-2 rounded-xl border border-zinc-700 bg-zinc-900 px-4 py-2.5 text-sm font-medium text-zinc-300 transition hover:border-zinc-500 hover:text-white disabled:opacity-50"
    >
      <Link2 size={15} />
      {loading ? "Connecting…" : "Connect Zerodha"}
    </button>
  );
}